...
int far_x1 = 20, far_x2 = 140, far_y1, far_y2;
...
void run_cross() {
    bool Xfound = Lpt0_found && Lpt1_found;
    // ?nt64_t current_encoder = get_total_encoder();
    ...
    if (cross_type == CROSS_BEGIN) {
        ...
        if (rpts0s_num < 5 && rpts1s_num < 5) { not_have_line++; }
...
//    far_x1 = cross_width, far_x2 = img_raw.width -cross_width;
    ...
    int x1 = img_bin.cols / 2 - begin_x, y1 = begin_y;
   ...
   {
    uint8_t *ptr = img_bin.ptr(y1);
    int current_value = ptr[x1-1];
    ...
    //先黑后白，先找white
    uint8_t *ptr = img_bin.ptr(y1);
    int current_value = ptr[far_x1];
    
    if (current_value >= 125) { 
        white_found = true; 
    }
    
    if (current_value < 125 && (white_found || far_x1 == cross_width)) {
        far_y1 = y1;
        break;
    }
}
    ...
    //从找到角点位置开始寻找
    uint8_t* ptr_left = img_bin.ptr(far_y1 + 1);
    int left_value = ptr_left[far_x1];

    if (left_value >= 125)
        findline_lefthand_adaptive(img_bin, far_x1, far_y1 + 1, far_ipts0, &far_ipts0_num);
    else {
        far_ipts0_num = 0;
    }

    int x2 = img_bin.cols / 2 + begin_x, y2 = begin_y;
    white_found = false;
    far_ipts1_num = sizeof(far_ipts1) / sizeof(far_ipts1[0]);

    //在begin_y向两边找黑线
   for(;x2<img_bin.cols-cross_width*2; x2++) 
   {
    uint8_t *ptr = img_bin.ptr(y2);
    int current_value = ptr[x2+1];
     if(current_value < 125) {
    ...
    //先黑后白，先找white
    uint8_t* ptr = img_bin.ptr(y2);
    int current_value = ptr[far_x2];
    
    if (current_value >= 125) { 
        white_found = true; 
    }
    
    if (current_value < 125 && (white_found || far_x2 == img_bin.cols - cross_width)) {
        far_y2 = y2;
        break;
    }
}
    ...
    uint8_t* ptr_right = img_bin.ptr(far_y2 + 1);
    int right_value = ptr_right[far_x2];

    if (right_value >= 125)
        findline_righthand_adaptive(img_bin, far_x2, far_y2 + 1, far_ipts1, &far_ipts1_num);
    else {
        far_ipts1_num = 0;
    }
    // 去畸变+透视变换
