...
int64_t total_encoder = 0;      // 累计编码器值


int64_t get_total_encoder() {
    // motor.update_encoders();  // 从硬件读取最新编码器值 
    int64_t current = motor.encoder1_counts;  // 读取硬件值（会自动清零）
...
void run_circle() {
    ...
    int64_t current_encoder = motor_l.total_encoder;
    ...
    if (circle_type == CIRCLE_LEFT_BEGIN) {
        ...
        if (rpts0s_num > 0.4 / sample_dist && none_left_line > 2) {
            have_left_line++;
            if (have_left_line > 1) {
    ...
    //走过圆环，寻右线
        ...
        //左线先丢后有
        if (rpts0s_num < 0.2 / sample_dist) { none_left_line++; }
        if (rpts0s_num > 0.4 / sample_dist && none_left_line > 3) {
    ...
    //右环控制，前期寻左直道
        ...
        if (rpts1s_num > 0.4 / sample_dist && none_right_line > 2) {
            have_right_line++;
